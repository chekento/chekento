...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){
        ...
        title("K ANDROID 1.0.2 · AUDIO BRIDGE",24,0xff58e6ff);
        title("Systemaudio · Claude/LLM speech · Music · 15-band one-row studio EQ · diagnostics",14,0xffff69c7);
        ...
        button("MIC / AMBIENT FALLBACK START",v->{ Intent s=new Intent(this,AudioBridgeService.class).setAction(AudioBridgeService.ACTION_MIC); if(Build.VERSION.SDK_INT>=26)startForegroundService(s);else startService(s); });
        button("STOP",v->{stopService(new Intent(this,AudioBridgeService.class));stopService(new Intent(this,OverlayTestService.class));});
        ...
        title("TEST zuerst: OVERLAY TEST starten und Home drücken. Die EQ-Zeile muss 15 Sekunden sichtbar pulsieren. OUTPUT liest danach Android-Playback ohne Umleitung. Wenn eine App Capture blockiert, nutze MIC FALLBACK.",12,0xffa9cbe2);
        refresh();
    }
    ...
    void refresh(){
        status.setText("Overlay: "+(Settings.canDrawOverlays(this)?"OK":"FEHLT")+" · Mic: "+(checkSelfPermission(Manifest.permission.RECORD_AUDIO)==PackageManager.PERMISSION_GRANTED?"OK":"optional")+" · Safe Zone: "+Math.round(sp.getFloat("safeH",.44f)*100)+" %");
        String stage=sp.getString("diag_stage","noch kein Test"), detail=sp.getString("diag_detail",""); String frames=String.valueOf(sp.getLong("diag_frames",0)); float rms=sp.getFloat("diag_rms",0f);
        diag.setText("Bridge: "+stage+(detail.length()>0?" · "+detail:"")+" · Frames "+frames+" · RMS "+String.format(java.util.Locale.US,"%.4f",rms));
    }
